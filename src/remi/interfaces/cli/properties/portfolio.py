...
async def _list_portfolios(manager_id: str | None, fmt_json: bool) -> None:
    ...
    for p in portfolios:
        ...
        items.append({
            "id": p.id, "name": p.name, "manager": manager.name if manager else "?",
            "properties": len(props), "description": p.description,
        })
    ...
    if fmt_json:
        ...
        if not items:
            ...
            return
        for p in items:
            typer.echo(f"  {p['id']:8s}  {p['name']:30s}  {p['manager']:20s}  {p['properties']} properties")
...
async def _summary(portfolio_id: str, fmt_json: bool) -> None:
    ...
    for prop in properties:
        ...
        total_revenue += rev
        prop_details.append({
            "id": prop.id, "name": prop.name, "type": prop.property_type.value,
