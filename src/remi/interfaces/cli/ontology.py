...
from remi.interfaces.cli.shared import get_container_async, json_out, parse_params, use_json
...
async def _search(
    ...
    results = await container.ontology_store.search_objects(
        type_name, filters=filters, order_by=order_by, limit=limit,
    )
    if fmt_json:
...
async def _related(
    ...
    if max_depth > 1:
        ...
        if fmt_json:
            json_out({"start": object_id, "depth": max_depth, "count": len(results), "nodes": results})
        else:
        ...
        if fmt_json:
            ...
            for lnk in links:
                typer.echo(f"  {lnk.get('source_id', '?')} --[{lnk.get('link_type', '?')}]--> {lnk.get('target_id', '?')}")
...
async def _aggregate(
    ...
    result = await container.ontology_store.aggregate(
        type_name, metric, field, filters=filters, group_by=group_by,
    )
    if fmt_json:
...
async def _timeline(
    ...
    events = await container.ontology_store.get_timeline(
        type_name, object_id, event_types=event_types, limit=limit,
    )
    if fmt_json:
        json_out({"object_type": type_name, "object_id": object_id, "count": len(events), "events": events})
    else:
        typer.echo(f"Timeline for {type_name}/{object_id}: {len(events)} events")
        for ev in events:
            typer.echo(f"  [{ev.get('timestamp', '?')}] {ev.get('event_type', '?')}: {ev.get('data', {})}")
...
async def _schema(type_name: str | None, fmt_json: bool) -> None:
    ...
    if type_name:
        ...
        if ot is None:
            json_out({"ok": False, "error": f"Unknown type '{type_name}'"}) if fmt_json else typer.echo(f"Unknown type: {type_name}")
            raise typer.Exit(1)
        data = ot.model_dump(mode="json")
        links = await container.ontology_store.list_link_types()
        related_links = [l.model_dump(mode="json") for l in links if l.source_type in (type_name, "*") or l.target_type in (type_name, "*")]
        ...
        if fmt_json:
            json_out({
                ...
                "link_types": [l.model_dump(mode="json") for l in links],
            })
        else:
            ...
            for l in links:
                typer.echo(f"  {l.name:20s} {l.source_type} -> {l.target_type}")
...
def codify(
    ...
    asyncio.run(_codify(knowledge_type, data, provenance, source_id, target_id, use_json(json_output)))
...
async def _codify(
    ...
    entity_id = await container.ontology_store.codify(
        knowledge_type, data_dict, provenance=provenance,
    )
...
async def _define(
    ...
    from remi.domain.ontology.types import KnowledgeProvenance, ObjectTypeDef, PropertyDef
