    ...
    *,
    ...
    if signal_store is not None:
        ...
        async def onto_signals(args: dict[str, Any]) -> Any:
            scope: dict[str, str] = {}
            if mid := args.get("manager_id"):
                scope["manager_id"] = mid
            if pid := args.get("property_id"):
                scope["property_id"] = pid
        ...
        registry.register(
            ...
            ToolDefinition(
                ...
                args=[
                    ToolArg(
                        name="manager_id",
                        description="Filter signals for a specific property manager",
                    ),
                    ToolArg(
                        name="property_id", description="Filter signals for a specific property"
                    ),
                    ToolArg(
                        name="severity",
    ...
    registry.register(
        ...
        ToolDefinition(
            ...
            args=[
                ...
                ToolArg(
                    name="filters",
                    description='Field filters as JSON object, e.g. {"status": "vacant"}',
                    type="object",
                ),
    ...
    registry.register(
        ...
        ToolDefinition(
            name="onto_related",
            description=(
                "Find objects related to a given object via link traversal in the ontology."
            ),
            args=[
                ToolArg(name="object_id", description="Object ID to start from", required=True),
                ToolArg(
                    name="link_type",
                    description="Filter by link type (e.g. BELONGS_TO, FOLLOWS, CAUSES)",
                ),
                ToolArg(
                    name="direction",
                ...
                ToolArg(
                    name="max_depth", description="Traversal depth (default: 1)", type="integer"
                ),
            ],
        ),
    )
    ...
    registry.register(
        ...
        ToolDefinition(
            name="onto_aggregate",
            description=(
                "Compute aggregate metrics (count, sum, avg, min, max) "
                "across objects in the ontology."
            ),
            args=[
                ToolArg(name="type_name", description="Object type", required=True),
                ToolArg(
                    name="metric", description="Metric: count, sum, avg, min, max", required=True
                ),
                ToolArg(
                    name="field", description="Field to aggregate on (required for sum/avg/min/max)"
                ),
    ...
    async def onto_schema(args: dict[str, Any]) -> Any:
        ...
        if type_name:
            ...
            links = [
                lt
                for lt in await store.list_link_types()
    ...
    registry.register(
        ...
        ToolDefinition(
            name="onto_schema",
            description=(
                "Describe an object type's properties and links, "
                "or list all types if no type_name given."
            ),
            args=[
    ...
    registry.register(
        ...
        ToolDefinition(
            ...
            description=(
                "Record a pattern or observation the agent noticed "
                "into the ontology knowledge graph."
            ),
            args=[
                ToolArg(
                    name="description", description="Description of the observation", required=True
                ),
                ToolArg(name="evidence", description="Supporting evidence"),
                ToolArg(
                    name="entity_refs", description="Comma-separated entity IDs this relates to"
                ),
            ],
        ),
    )
    ...
    registry.register(
        ...
        ToolDefinition(
            ...
            description=(
                "Define a new object type in the ontology to extend the schema for discovered data."
            ),
            args=[
                ...
                ToolArg(
                    name="properties",
                    description="Property definitions as JSON array",
                    type="object",
                ),
            ],
        ),
