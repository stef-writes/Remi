...
from remi.domain.signals.types import Provenance, Severity, Signal
    ...
    def _detect_outliers(
        ...
        for entity_id, val in values:
            ...
            if abs(zscore) >= self._zscore_threshold:
                ...
                severity = (
                    Severity.HIGH if abs(zscore) >= 3.5
                    else Severity.MEDIUM if abs(zscore) >= 3.0
                    else Severity.LOW
                )
            ...
            if pct >= self._concentration_threshold:
                signals.append(Signal(
                    signal_id=_signal_id(
                        "concentration", type_name, field_name,
                        value.replace(" ", "_")[:20],
                    ),
