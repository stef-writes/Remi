...
class PatternDetector:
            ...
            try:
                ...
                for field_name in numeric_fields:
                    hyps = self._detect_threshold_patterns(
                        objects, type_def.name, field_name,
                    )
                    for h in hyps:
                ...
                categorical = _identify_categorical_fields(
                    objects, exclude=numeric_fields,
                )
    ...
    def _detect_threshold_patterns(
        ...
        outlier_count = sum(
            1 for _, v in values
            if abs((v - mean) / stddev) >= self._zscore_threshold
        )
        ...
        return [Hypothesis(
            ...
            title=(
                f"Propose threshold signal for {type_name}.{field_name}"
            ),
            description=(
    ...
    def _detect_correlations(
        ...
        return [Hypothesis(
            hypothesis_id=_hypothesis_id(
                "correlation", f"{type_name}.{field_a}_{field_b}",
            ),
        ...
        for value, count in counts.items():
            ...
            if pct >= self._concentration_threshold:
                ...
                hypotheses.append(Hypothesis(
                    ...
                    title=(
                        f"Concentration: {pct:.0%} of {type_name} "
                        f"have {field_name}='{value}'"
                    ),
                    description=(
...
def _extract_numeric_values(
    objects: list[dict[str, Any]], field_name: str,
