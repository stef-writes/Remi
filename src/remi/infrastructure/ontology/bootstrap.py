...
_CORE_TYPES: list[ObjectTypeDef] = [
    ...
    ObjectTypeDef(
        ...
        properties=(
            ...
            PropertyDef(
                name="property_type", data_type="enum",
    ...
    ObjectTypeDef(
        ...
        properties=(
            ...
            PropertyDef(
                name="category", data_type="enum",
                enum_values=["plumbing", "electrical", "hvac", "appliance", "structural", "general", "other"],
            ),
            PropertyDef(
                name="priority", data_type="enum",
...
_STRUCTURAL_LINKS: list[LinkTypeDef] = [
    LinkTypeDef(name="BELONGS_TO", source_type="Unit", target_type="Property",
                cardinality="many_to_one", description="Unit is part of a property"),
    LinkTypeDef(name="IN_PORTFOLIO", source_type="Property", target_type="Portfolio",
                cardinality="many_to_one", description="Property belongs to a portfolio"),
    LinkTypeDef(name="MANAGED_BY", source_type="Property", target_type="PropertyManager",
                cardinality="many_to_one", description="Property is managed by"),
    LinkTypeDef(name="COVERS", source_type="Lease", target_type="Unit",
                cardinality="many_to_one", description="Lease covers a unit"),
    LinkTypeDef(name="SIGNED_BY", source_type="Lease", target_type="Tenant",
                cardinality="many_to_one", description="Lease signed by tenant"),
    LinkTypeDef(name="AFFECTS", source_type="MaintenanceRequest", target_type="Unit",
                cardinality="many_to_one", description="Work order affects a unit"),
]

_OPERATIONAL_LINKS: list[LinkTypeDef] = [
    LinkTypeDef(name="CAUSES", source_type="*", target_type="*",
                description="Causal relationship", provenance=KnowledgeProvenance.SEEDED),
    LinkTypeDef(name="LEADS_TO", source_type="*", target_type="*",
                description="Outcome chain", provenance=KnowledgeProvenance.SEEDED),
    LinkTypeDef(name="MITIGATED_BY", source_type="*", target_type="*",
                description="Risk mitigation", provenance=KnowledgeProvenance.SEEDED),
    LinkTypeDef(name="FOLLOWS", source_type="*", target_type="*",
                description="Workflow step ordering", provenance=KnowledgeProvenance.SEEDED),
    LinkTypeDef(name="TRIGGERS", source_type="*", target_type="*",
                description="Event triggers action", provenance=KnowledgeProvenance.SEEDED),
    LinkTypeDef(name="REQUIRES", source_type="*", target_type="*",
                description="Prerequisite relationship", provenance=KnowledgeProvenance.SEEDED),
    LinkTypeDef(name="MEASURED_BY", source_type="*", target_type="*",
                description="Entity measured by a metric", provenance=KnowledgeProvenance.SEEDED),
    LinkTypeDef(name="AFFECTS_METRIC", source_type="*", target_type="*",
                description="Process affects a metric", provenance=KnowledgeProvenance.SEEDED),
    LinkTypeDef(name="ESCALATES_TO", source_type="*", target_type="*",
                description="Escalation path", provenance=KnowledgeProvenance.SEEDED),
    LinkTypeDef(name="OWNED_BY_ROLE", source_type="*", target_type="*",
                description="Workflow owned by role", provenance=KnowledgeProvenance.SEEDED),
]
...
async def bootstrap_ontology(
    ...
    for chain in tbox.get("causal_chains", []):
        ...
        await store.codify("cause", {"description": description},
                           provenance=KnowledgeProvenance.SEEDED)
        await store.put_link(source_id, "CAUSES", target_id,
...
async def _seed_workflow(
    ...
    for step_id, description in steps:
        await store.put_object("process", step_id, {
