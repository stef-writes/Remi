...
class BridgedOntologyStore(OntologyStore):
        ...
        *,
        ...
        if self._is_core(type_name):
            ...
            entities = await self._ks.find_entities(
                _NS, entity_type=type_name, limit=limit * 2
            )
    ...
    async def aggregate(
        ...
        *,
        ...
        items = await self.search_objects(
            type_name, filters=filters, limit=10_000
        )
    ...
    async def get_timeline(
        ...
        *,
        ...
        all_events = await self._ks.find_entities(
            _NS, entity_type="event", limit=limit * 5
        )
        results = []
        for e in all_events:
            if (
