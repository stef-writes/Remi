...
Used by:
  - Sandbox scripts (via the seeded remi_client module)
...
class RemoteOntologyStore(OntologyStore):
    ...
    async def search_objects(
        ...
        *,
        ...
        if filters:
            data = await self._post(f"/search/{type_name}", {
                "filters": filters,
                "order_by": order_by,
                "limit": limit,
            })
    ...
    async def aggregate(
        ...
        *,
        ...
        data = await self._post(f"/aggregate/{type_name}", {
            "metric": metric,
            "field": field,
            "filters": filters,
            "group_by": group_by,
