...
class InMemoryKnowledgeStore(KnowledgeStore):
    ...
    async def store(
        ...
        self._data[namespace][key] = MemoryEntry(
            namespace=namespace, key=key, value=value, created_at=datetime.now(UTC),
        )
        ...
        *,
        ...
        while queue:
            ...
            rels = await self.get_relationships(
                namespace, current_id, direction="outgoing"
            )
            for rel in rels:
    ...
    async def delete_entity(self, namespace: str, entity_id: str) -> bool:
        if entity_id in self._entities.get(namespace, {}):
            ...
            self._relationships[namespace] = [
                r for r in self._relationships.get(namespace, [])
