...
from remi.domain.ingestion.context import IngestionCtx, ensure_property, link, merge_kb
from remi.domain.ingestion.resolver import (
    LEASE_END_FALLBACK,
...
async def persist_unit(row: dict[str, Any], ctx: IngestionCtx) -> None:
    ...
    status = (UNIT_STATUS_FROM_OCCUPANCY.get(occ, UnitStatus.VACANT)
              if occ else UnitStatus.VACANT)

    unit = Unit(
        id=uid, property_id=prop_id, unit_number=unum,
        status=status, occupancy_status=occ,
        bedrooms=to_int(row.get("bedrooms")),
        bathrooms=(float(row["bathrooms"])
                   if row.get("bathrooms") is not None else None),
        sqft=to_int(row.get("sqft")),
        market_rent=to_decimal(row.get("market_rent")),
        current_rent=to_decimal(
            row.get("monthly_rent") or row.get("current_rent")),
    ...
    kb: dict[str, str | int | float | bool | None] = {
        "property_id": prop_id, "unit_number": unum,
        "status": status, "source_doc": ctx.doc_id,
    }
    if occ is not None:
...
async def persist_tenant(row: dict[str, Any], ctx: IngestionCtx) -> None:
    ...
    prop_id = await ensure_property(
        row, ctx, replace_units=True, replace_leases=True)
    unum = str(row.get("unit_number") or "main").strip()
    uid = slugify(f"unit:{prop_id}:{unum}")
    ...
    await merge_kb(ctx, uid, f"{ctx.platform}_unit", {
        "property_id": prop_id, "unit_number": unum,
    ...
    tenant = Tenant(
        id=tid, name=tname,
        status=TENANT_STATUS_MAP.get(str(raw_st).strip().lower(),
                                     TenantStatus.CURRENT),
        balance_owed=to_decimal(
            row.get("amount_owed") or row.get("balance_owed")),
    ...
    await merge_kb(ctx, lid, f"{ctx.platform}_lease", {
        "unit_id": uid, "tenant_id": tid, "property_id": prop_id,
...
async def _persist_delinquency_notes(
    ...
    raw = str(
        row.get("notes")
        or row.get("delinquency_notes")
        or row.get("delinquent_notes")
        or ""
    ).strip()
    if not raw:
        return
    ...
    for idx, line in enumerate(lines):
        ...
        await merge_kb(ctx, nid, "delinquency_note", {
            "content": line,
            "entity_type": "Tenant",
            "entity_id": tenant_id,
...
async def persist_lease(row: dict[str, Any], ctx: IngestionCtx) -> None:
    ...
    await ctx.ps.upsert_unit(Unit(
        ...
        sqft=to_int(row.get("sqft")),
        market_rent=to_decimal(row.get("market_rent")),
        current_rent=rent, source_document_id=ctx.doc_id,
    ))
    await merge_kb(ctx, uid, f"{ctx.platform}_unit", {
        "property_id": prop_id, "unit_number": unum,
        "status": UnitStatus.OCCUPIED if active else UnitStatus.VACANT,
    ...
    await ctx.ps.upsert_tenant(Tenant(
        id=tid, name=tname, status=TenantStatus.CURRENT,
        phone=(str(row.get("phone_numbers") or row.get("phone") or "")
               .strip() or None),
        source_document_id=ctx.doc_id,
    ))
    await merge_kb(ctx, tid, f"{ctx.platform}_tenant", {
        "name": tname, "status": TenantStatus.CURRENT,
        "source_doc": ctx.doc_id,
    })
    ctx.result.entities_created += 1
...
async def persist_maintenance(
    ...
    prop_id = await ensure_property(row, ctx)
    unum = str(
        row.get("unit_number") or row.get("unit_id") or "main").strip()
    uid = slugify(f"unit:{prop_id}:{unum}")
    ...
    mp: dict[str, str | int | float | bool | None] = {
        "unit_id": uid, "property_id": prop_id,
...
async def persist_owner(row: dict[str, Any], ctx: IngestionCtx) -> None:
    ...
    await merge_kb(ctx, oid, f"{ctx.platform}_owner", {
        "name": name, "source_doc": ctx.doc_id,
    })
    ctx.result.entities_created += 1
...
async def persist_vendor(row: dict[str, Any], ctx: IngestionCtx) -> None:
    ...
    vendor = Vendor(
        id=vid,
        name=name,
        category=(VendorCategory(cat_raw)
    ...
    await merge_kb(ctx, vid, f"{ctx.platform}_vendor", {
        "name": name, "category": vendor.category,
        "is_internal": vendor.is_internal, "source_doc": ctx.doc_id,
    })
    ctx.result.entities_created += 1
